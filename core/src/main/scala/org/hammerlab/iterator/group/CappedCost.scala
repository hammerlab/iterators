...
class CappedCost[T](it: BufferedIterator[T]) {
  ...
  private def cappedCostGroup[N: Numeric](withCosts: BufferedIterator[(T, N)], costFn: T ⇒ N, limit: N)(
              ...
              case (elem, curCost) ⇒
                ...
                if (lteq(cost + curCost, limit)) {
                  ...
                  elementTooCostlyStrategy match {
                    case ElementTooCostlyStrategy.Discard ⇒
                      withCosts.next
                      _advance
                    case ElementTooCostlyStrategy.EmitAlone ⇒
                      take
                    case ElementTooCostlyStrategy.Throw ⇒
