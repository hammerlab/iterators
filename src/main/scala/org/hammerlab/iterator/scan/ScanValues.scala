...
trait ScanValues {
    implicit class ScanValuesOps[K, V](it: Iterator[(K, V)]) extends Serializable {
  ...
  implicit def makeScanValuesIterator[K, V](it: Iterable[(K, V)]): ScanValuesOps[K, V] = ScanValuesOps(it.iterator)
  implicit def makeScanValuesIterator[K, V](it: Array[(K, V)]): ScanValuesOps[K, V] = ScanValuesOps(it.iterator)
}
